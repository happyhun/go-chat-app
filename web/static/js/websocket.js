...
export function connectLobbyStream(nickname) {
    if (state.lobbyEventSource && state.lobbyEventSource.readyState !== EventSource.CLOSED) {
    ...
    state.lobbyEventSource.onmessage = (event) => {
        ...
        try {
            ...
            document.dispatchEvent(new CustomEvent('display-rooms', {detail: rooms}));
        } catch (e) {
...
export function connectWebSocket() {
    ...
    const socketURL = `${protocol}://${window.location.host}/ws/${encodeURIComponent(state.roomID)}`;
    state.socket = new WebSocket(socketURL);

    state.socket.onopen = () => {
        state.socket.send(JSON.stringify({type: MSG_TYPES.REGISTER, content: state.myNickname}));
    };
    ...
    state.socket.onclose = (event) => {
        ...
        let messageForUser = "Connection to the server has been lost. Returning to the lobby.";
        let delay = 2000;

        if (event.code === 1006) { // Abnormal Closure
            messageForUser = "Could not connect to the chat room. It may not exist or has been deleted.";
