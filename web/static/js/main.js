...
import {appendChatMessage, displayRooms, showLobbyLoader, showView, updateCharCounter} from './ui.js';
import {connectLobbyStream, connectWebSocket, sendTypingStop} from './websocket.js';
...
async function handleNicknameSubmit() {
    ...
    const validationError = validateInput(nickname, state.config.minNicknameLength, state.config.maxNicknameLength, /^[a-zA-Z0-9가-힣_]+$/, "Nickname");
...
function submitChatMessage() {
    ...
    if (message.length > state.config.maxChatMessageLength) {
        alert(`Message cannot exceed ${state.config.maxChatMessageLength} characters.`);
        return;
    }
    if (message.trim() && state.socket && state.socket.readyState === WebSocket.OPEN) {
        state.socket.send(JSON.stringify({type: MSG_TYPES.CHAT, content: message.trim()}));
        sendTypingStop();
        appendChatMessage({nickname: state.myNickname, content: message.trim(), timestamp: new Date().toISOString()});
...
async function handleNicknameInput() {
    ...
    const validationError = validateInput(trimmedNickname, state.config.minNicknameLength, state.config.maxNicknameLength, /^[a-zA-Z0-9가-힣_]+$/, "Nickname");
    if (validationError) {
...
function setupEventListeners() {
    ...
    ui.createRoomForm.addEventListener("submit", async (e) => {
        ...
        const validationError = validateInput(roomID, 1, state.config.maxRoomIDLength, /^[a-zA-Z0-9가-힣_-]+$/, "Room Name");
    ...
    document.addEventListener("visibilitychange", () => {
        ...
        if (state.currentView === 'lobby' && (!state.lobbyEventSource || state.lobbyEventSource.readyState === EventSource.CLOSED)) {
            console.log("Reconnecting lobby stream...");
            connectLobbyStream(state.myNickname);
        }
        if (state.currentView === 'chat' && (!state.socket || state.socket.readyState === WebSocket.CLOSED)) {
