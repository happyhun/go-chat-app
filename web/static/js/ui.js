...
export function showView(viewName, focusElement = null) {
    ["nickname", "lobby", "chat"].forEach(id => ui[`${id}Container`].classList.add("hidden"));
...
export function displayRooms(rooms) {
    ...
    if (rooms && rooms.length > 0) {
        rooms.forEach(room => {
            ...
            card.setAttribute("aria-label", `Join room ${room.id}, currently ${room.users} users`);
            ...
            const join = () => document.dispatchEvent(new CustomEvent('join-room', {detail: room.id}));
            card.onclick = join;
            card.onkeydown = (e) => {
...
export function updateTypingIndicator() {
    ...
    ui.typingIndicator.textContent = `${typers.join(', ')} and ${typers.length - 2} others are typing...`;
}
