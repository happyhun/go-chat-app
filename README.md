...
| HTTP Method | 경로                  | 설명                                                 |
|-------------|-----------------------|------------------------------------------------------|
| `GET`       | `/`                   | 메인 애플리케이션 페이지 (`index.html`) 제공         |
| `GET`       | `/static/*`           | 정적 파일 (CSS, JS) 제공                             |
| `GET`       | `/api/config`         | 클라이언트 설정 (메시지 길이 제한 등) 반환         |
| `POST`      | `/api/nicknames/check`| 닉네임 사용 가능 여부 확인                           |
| `GET`       | `/api/rooms`          | 현재 활성화된 채팅방 목록 반환                       |
| `POST`      | `/api/rooms`          | 새로운 채팅방 생성                                   |
| `GET`       | `/api/rooms/stream`   | 로비 업데이트를 위한 SSE(Server-Sent Events) 스트림 |
| `GET`       | `/ws/{roomID}`        | 특정 채팅방의 웹소켓 연결 처리                     |
